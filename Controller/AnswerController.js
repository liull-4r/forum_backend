...
const postanswer = async (req, res) => {
    ...
    if (!answer || !questionid) {
      ...
      try {
        const [questions] = await connection.query("SELECT * FROM questions WHERE questionid = ?", [questionid]);
        if (questions.length === 0) {
            return res.status(StatusCodes.BAD_REQUEST).json({ message: "Invalid question ID" });
        }
    } catch (error) {
        // console.error(error.message);
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({message:"Something went wrong while verifying question ID, try again later"});
    }

    const userid = req.user.userid; 

    try {
        await connection.query("INSERT INTO answers (answer, userid, questionid) VALUES (?, ?, ?)", [answer, userid, questionid]);
...
const getanswer = async (req, res) => {
    ...
    try {
        const [answer] = await connection.query("SELECT * FROM answers JOIN users ON answers.userid = users.userid WHERE questionid = ?", [questionid]);
